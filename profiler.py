"""
profiler.py — Engine frame-time profiler (deep edition)
========================================================

Usage basique (identique à avant) :
    from profiler import ProfiledRun
    ProfiledRun(engine, on_update=my_update, frames=300).run()

Nouveau — introspection automatique de la scène :
    ProfiledRun(engine, ..., deep=True).run()
    # Wrappe automatiquement toutes les méthodes update/draw des objets de la scène

Nouveau — décorateur manuel pour descendre encore plus loin :
    from profiler import profile_section

    class MySystem:
        @profile_section("physics.collisions")
        def update(self, dt):
            ...
"""
...
# ─────────────────────────────────────────────────────────────── thread-local
# Permet au décorateur @profile_section de trouver le Profiler actif
# sans le passer en argument partout.
import threading
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            with prof.track(name):
                return fn(*args, **kwargs)
        wrapper._profile_section = name   # marqueur pour éviter le double-wrap
        return wrapper
    return decorator
...
class Profiler:
    ...
    def __init__(self):
        ...
        self._patched: list[tuple[object, str, object]] = []   # (obj, attr, original)
    ...
    # Catégories de sections "parent" déjà trackées — on ne wrappe pas deux fois
    ...
    def instrument_scene(self, scene, prefix: str = "scene") -> None:
        ...
        Args:
            scene:   l'objet racine (eng.scene, une liste de systèmes, etc.)
            prefix:  préfixe affiché dans le rapport (ex: "scene")
        """
    ...
    def _walk_and_patch(self, obj, prefix: str, depth: int, visited: set) -> None:
        ...
        for method_name in self._TARGET_METHODS:
            raw = cls.__dict__.get(method_name)          # uniquement défini ici, pas hérité
            if raw is None:
                continue
    ...
    def _patch_method(self, obj, method_name: str, section_name: str) -> None:
        ...
        try:
            ...
            pass   # objets en lecture seule ou slots → on skip silencieusement
    ...
    def report(self, group_by_prefix: bool = True) -> str:
        ...
        # On identifie les "groupes" = premier niveau du nom (ex: "scene", "update")
        ...
        if group_by_prefix:
            # Construire un dict group → items
            from collections import defaultdict
            groups: dict[str, list] = defaultdict(list)
            for name, stats in ranked:
                groups[group_key(name)].append((name, stats))

            # Trier les groupes par mean cumulée décroissante
            group_totals = {
                g: sum(s.mean for _, s in items)
            ...
            for g in sorted_groups:
                ...
                if len(items) == 1:
                    # Groupe avec un seul élément → ligne simple
                    name, stats = items[0]
                    ...
                    # Groupe avec plusieurs éléments → ligne de groupe + sous-lignes indentées
...
class ProfiledRun:
    ...
    Args:
        ...
        deep:         si True, introspecte et wrappe automatiquement les objets
                      de eng.scene pour descendre au niveau classe/méthode
        scene_roots:  liste d'objets supplémentaires à inspecter en mode deep
                      (ex: [eng.physics, eng.audio])
    """
    ...
    def run(self) -> None:
        ...
        # ── active profiler (pour @profile_section) ───────────────────────
